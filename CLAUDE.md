# CLAUDE
<!-- hyperresearch:start -->
## Research Base (hyperresearch) — Today is 2026-06-27

**CLI path: `/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch`** — use this exact path for every hyperresearch command. It may not be on your system PATH.
...
**Do NOT use WebFetch for source pages** — use `/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch fetch` instead. The skill files explain when to fetch vs. search.
...
`/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch fetch` auto-detects PDF URLs (arXiv, NBER, SSRN, direct `.pdf` links) and extracts full text via pymupdf. Fetch them aggressively. Raw PDFs land in `research/raw/<note-id>.pdf` and the note's frontmatter links back via `raw_file:`.

### Searching the vault

```bash
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch search "query" --json                # Full-text search
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch search "query" --tag ml --json       # Filter by tag / status / date / parent
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch search "query" --include-body --json # Full-body search, not just titles
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch note show <id> --json                # Read one note
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch note show <id1> <id2> <id3> --json   # Batch-read notes in one call
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch note list --json                     # List all notes with summaries
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch tags --json                          # Existing tag vocabulary
```

### Images, screenshots, and assets

```bash
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch fetch "<url>" --tag <topic> --save-assets -j   # Saves screenshot + top images
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch assets list --note <note-id> --json            # Assets for a specific note
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch assets path <note-id> --type screenshot -j     # Get screenshot path (viewable with Read)
```

### Authenticated crawling

Login-gated content (LinkedIn, Twitter, paywalled news) needs a browser profile. Set up once via `/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch setup` or `crwl profiles`. Config in `.hyperresearch/config.toml` under `[web]`: `profile = "research"`, `magic = true`. LinkedIn / Twitter / Facebook / Instagram / TikTok auto-use a visible browser to avoid session kills.

If a fetch returns a login wall, tell the user to run `/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch setup` and create a login profile.
...
```bash
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch note list --status draft -j                                        # Find unprocessed notes
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch note show <id> -j                                                  # Read the content
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch note update <id> --summary "<specific summary>" --add-tag <t> -j   # Add summary + tags
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch lint -j                                                            # Find missing tags / summaries / broken links
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch repair -j                                                          # Auto-fix broken links, rebuild indexes
/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch status -j                                                          # Overall vault health
```

Lifecycle: `draft` → `review` → `evergreen` (or `stale` → `deprecated` → `archive` for outdated material).

Summaries must be specific — "Mamba achieves linear-time sequence modeling via selective state spaces" beats "Paper about Mamba". Reuse the existing tag vocabulary (`/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch tags -j`) rather than inventing new tags.

### Key conventions

- Notes live in `research/notes/` as markdown with YAML frontmatter
- Link notes with `[[note-id]]` syntax
- After editing `.md` files directly, run `/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch sync` to update the index
- Run `/home/codeseys/.local/share/uv/tools/hyperresearch/bin/hyperresearch --help` for the full command list
<!-- hyperresearch:end -->
