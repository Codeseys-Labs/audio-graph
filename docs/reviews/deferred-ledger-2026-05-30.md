...
|------|-------|------------------------------------------|
| **B15** OpenAI Realtime STT | client + parser + reconnect + settings + dispatch; 8 verbatim-JSON parser tests | Configure an OpenAI Realtime credential through Express Setup or the local credential backend, then live-smoke the WS handshake / transcript streaming / reconnect. |
...
| **B18** native S2S (Gemini AUDIO + turn FSM) | (Wave 3a) AUDIO config + event decode + pure FSM with gating | Configure a Gemini credential through Express Setup or the local credential backend, then test with an audio playback device: real barge-in, AEC, audio-out. |
