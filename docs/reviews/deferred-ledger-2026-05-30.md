...
|------|-----------------------------------|-----------------|
| **B21** edition 2021→2024 | **Platform gate REMOVED 2026-05-31** — earlier entries wrongly called this "needs a multi-OS CI matrix we don't have." We DO: CI runs Linux + Windows + macOS Blacksmith runners (`rust-{linux,windows,macos}`) with `cargo test`, and PR #14 is green on all three. The real remaining work is the *migration effort*: hand-audit the **24 verified `tail_expr_drop_order`/`if_let_rescope` sites** (`docs/plans/b21-edition-2024-migration-plan.md`, surfaced via the lint in WSL) with Pattern A–D rewrites — each changes lock-release / channel-disconnect timing, so each needs individual review — then flip `edition=2024`. `cargo fix` does NOT auto-fix this lint. | A focused implementation pass: apply the 24 Pattern A–D rewrites + flip the edition, push, and read the all-platform CI (which already runs `cargo test` per OS). No longer platform-blocked — it's scoped effort + a CI run on the migration branch. Locally verifiable too: native Windows `cargo test` (manifest env var) + WSL Linux. |
