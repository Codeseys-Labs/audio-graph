...
- `DeepgramStreamingClient::send_audio` (`asr/deepgram.rs:363-410`) converts f32→i16 LE PCM, queues on an **unbounded** tokio mpsc; a writer task drains to binary WS frames. Backpressure cap: `AUDIO_BUFFER_MAX_CHUNKS = 200` (`deepgram.rs:173`, ~10 s of audio) — over the cap it flips `user_disconnected` and errors (`deepgram.rs:4390-4396`). Keepalive text frame every 4 s idle (`KEEPALIVE_INTERVAL_SECS = 4`, `deepgram.rs:176-177`).
