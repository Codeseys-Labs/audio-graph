...
**For a structured JSON KG specifically, the calculus flips toward JSON-Patch (RFC 6902).** The `json-correction-loop` result is stark (a single project's benchmark, not a universal figure — one 100-entity / 183-edge JSON-KG fixture on `gpt-4o-mini`, snapshot per the `github/warpspaceinc/json-correction-loop` repo at time of research): full-regeneration on critic feedback fixes **0/8** flagged defects and burns 73K tokens, while a surgical RFC-6902 patch loop fixes **8/8 at ~5× fewer tokens** [github/warpspaceinc/json-correction-loop]. Treat the exact ratios as illustrative of that fixture, not a general law. Two design lessons from that project: (i) restrict the patch vocabulary — they implement only `add`/`replace`/`remove` and *reject* `move`/`copy` by design; (ii) naive patching alone only fixes 35–64% because the LLM patches the *symptom* (edge with wrong predicate) not the *root cause* (entity whose type flipped) — a `path_finder` sub-agent that redirects symptom→root-cause pointers is what closes the gap to 100%.
