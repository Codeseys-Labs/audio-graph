...
1. **Add bitemporal validity to notes.** Give `MaterializedNote` `valid_from_ms`/`valid_until_ms` (mirror `MaterializedGraphNode`) and add an `InvalidateNote { id }` op so a note can be SOFT-superseded (hidden but auditable) instead of hard `DeleteNote`/overwrite. Trivial — copy the graph node's invalidate path (`projections.rs:1545`, `1725`).
