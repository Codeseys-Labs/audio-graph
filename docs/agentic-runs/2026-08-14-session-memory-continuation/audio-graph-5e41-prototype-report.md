...
directory-entry barrier for newly created streams, and refuses to infer an
in-flight remote request's external result after restart. It neither calls nor
imports production code and makes no operating-system durability claim.
...
- only `Accepted` or exact-retry `AlreadyAccepted` atomically establishes the
  committed sequence, materialized sequence, and basis-eligible sequence;
...
- deletion fences every writer before either immediate discard or a wait that
  observes but never applies remote terminals.
...
barrier. The model uses abstract proof flags only; `audio-graph-8e73` still owns
...
total cases:                  744
reducer transitions:        7,434
unique full states:           936
invariant assertions:      55,638
invariant families:            23
```

All five receipts were observed. The 23 passing invariant families were:
...
guards are not included in the 23 post-transition invariant-family count.
...
- `audio-graph-3b48` owns durable scheduler enqueue, exact job/attempt recovery,
  independent lane progression, and accepted remote-reissue policy.
- `audio-graph-90f3` owns the production `Pending` to receipt-bearing canonical
  ...
  advancement, and snapshot-cache authority.
...
- `audio-graph-9c89` and later `audio-graph-e969` own Session rotation/load and
  ...
  and report residual artifacts exactly.
...
```text
cases explored: 744
transitions evaluated: 7434
unique full states observed: 936
invariant assertions: 55638
invariant families passed (23)
...
docs/prototypes/session-projection-admission-and-fencing.md
scripts/prototype-session-projection-admission.mjs
A  docs/agentic-runs/2026-08-14-session-memory-continuation/audio-graph-5e41-prototype-report.md
A  docs/prototypes/session-projection-admission-and-fencing.md
A  scripts/prototype-session-projection-admission.mjs
```

Result: pass, exit 0. The staged footprint is exactly the three assigned new
files. `HEAD` and its merge base with the assigned base were both
`72e23b506d6f4d2e465aeebfb452d20fbbc0bfe5` before commit.
...
- Waiting during deletion changes latency, not write safety: the fence is
  mandatory and observed remote results remain unusable under both policies.
