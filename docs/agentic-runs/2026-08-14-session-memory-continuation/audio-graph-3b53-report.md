...
- "Re-keying is therefore blocked on a binding that does not exist yet, owned by
  ...
  (`src-tauri/src/persistence/session_artifact_manifest.rs:2499`), which reads the
...
- "When the binding exists, that test should be inverted rather than deleted" —
  ...
  (`src-tauri/src/persistence/session_artifact_manifest.rs:4317`), in the same
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:2464`) — is still exactly
