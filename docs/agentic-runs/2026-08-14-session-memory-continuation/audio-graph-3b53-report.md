...
- "Re-keying is therefore blocked on a binding that does not exist yet, owned by
  ...
  (`src-tauri/src/persistence/session_artifact_manifest.rs:2392`), which reads the
...
- "When the binding exists, that test should be inverted rather than deleted" —
  ...
  (`src-tauri/src/persistence/session_artifact_manifest.rs:4146`), in the same
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:2357`) — is still exactly
