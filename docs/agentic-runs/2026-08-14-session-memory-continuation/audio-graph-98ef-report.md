...
- Deepgram Nova effective fidelity follows selected diarization, speaker-cap,
  VAD, endpointing, and utterance-end settings. A speaker cap reports app-owned
  remapping rather than provider-owned speaker fidelity.
...
- Deepgram readiness cache fingerprints now include only non-secret selected
  fidelity controls. Config changes cannot reuse a stale effective-fidelity
  result, and no API key enters the fingerprint or response.
...
The Rust baseline/set tests also passed, including the new exact Wave 5 literal
and the existing derived `ui_selectable` set equality test. No provider was
promoted.
...
deepgram_effective_fidelity_uses_selected_model_diarization_and_turn_controls: 1 passed; 0 failed
...
- `betterleaks dir --no-banner --redact <touched files and report>`
  - PASS: approximately 1.10 MB scanned, no leaks found.
