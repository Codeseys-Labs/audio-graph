...
(`src-tauri/src/persistence/session_artifact_manifest.rs:2101`) ends with two
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:2129`). That is internal
...
1. **The durable proof does not exist yet when `prepare_compare_and_swap` runs on
   ...
   `src-tauri/src/persistence/session_artifact_manifest.rs:1245` and only then
   proves the durable record equals the proof bytes at
   `src-tauri/src/persistence/session_artifact_manifest.rs:1255`. So the binding
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:1688`) and the
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:2164`), so the diff does
not abut `commit_prepared_compare_and_swap`. Bound constant at
`src-tauri/src/persistence/session_artifact_manifest.rs:2146` (u64 **[deviation]**,
...
3. Open read-only, **then re-check the opened handle's metadata** — `is_file`,
   ...
   (`src-tauri/src/persistence/session_artifact_manifest.rs:1888`) but mirrored
   only its first half; the precedent continues at
   `src-tauri/src/persistence/session_artifact_manifest.rs:1904` with exactly this
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:2216`): `NotFound` from
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:1530`) is called from
`prepare_compare_and_swap` at
`src-tauri/src/persistence/session_artifact_manifest.rs:1596`, replacing the
...
1. `recovery_key(&candidate.transition.fingerprint)`
   (`src-tauri/src/persistence/session_artifact_manifest.rs:1680`) is therefore
...
2. **[critique 3]** `IdempotencyConflict`
   (`src-tauri/src/persistence/session_artifact_manifest.rs:1629`) is unreachable
   ...
   (`src-tauri/src/persistence/session_artifact_manifest.rs:1642`). The refusal
...
| --- | --- | --- |
| T1 | `committed_v2_head_records_later_generations_only_against_the_durable_proof` (`src-tauri/src/persistence/session_artifact_manifest.rs:3712`) | invert the pinning test rather than deleting it; B4 liveness closed |
| T2 | `forged_v2_provenance_is_refused_on_an_advanced_head` (`src-tauri/src/persistence/session_artifact_manifest.rs:3913`) | the seed's required RED: forged identity / fabricated fingerprint refused **after** the head advanced |
| T3 | `advanced_head_refuses_later_generations_when_the_durable_proof_is_not_intact` (`src-tauri/src/persistence/session_artifact_manifest.rs:4022`) | provably tied to the durable record, not merely self-consistent |
| T4 | `addressed_generic_cas_cannot_install_v2_without_owning_proof_bytes` (`src-tauri/src/persistence/session_artifact_manifest.rs:3537`), one case added | a real transition still requires its proof |
| T5 | `advanced_head_conflicts_on_the_same_idempotency_id_with_changed_artifacts` (`src-tauri/src/persistence/session_artifact_manifest.rs:3782`) | same idempotency id with changed artifacts still conflicts |
| T6 | `quarantine_recovery_remains_closed_on_a_v2_session` (`src-tauri/src/persistence/session_artifact_manifest.rs:3830`) | the "also record while here" closure, as executable evidence |
| — | `accepted_v2_manifest_cannot_regress_to_v1` (`src-tauri/src/persistence/session_artifact_manifest.rs:3436`) plus T1's third assertion | a V1 candidate against a V2 head is still a floor regression |
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:1648`). The test comment
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:3464` — its second call is
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:4119`),
`stale_existing_head_rejects_different_transition_before_proof_mutation`
(`src-tauri/src/persistence/session_artifact_manifest.rs:4204`),
`v2_candidate_requires_exact_bound_session_provenance_proof`
(`src-tauri/src/persistence/session_artifact_manifest.rs:3292`),
