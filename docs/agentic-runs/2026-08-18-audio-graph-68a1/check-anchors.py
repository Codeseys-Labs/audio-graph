...
EXPECTED: dict[tuple[str, int], str] = {
    ...
    (MANIFEST, 692): "V2ProvenanceProofBinding(V2ProvenanceProofBindingError),",
    (MANIFEST, 1650): "fn refuse_unproven_v2_candidate(",
    (MANIFEST, 1716): "self.refuse_unproven_v2_candidate(&candidate, proof_owned)",
    (MANIFEST, 2431): "pub(crate) struct V2SessionProvenanceEntry<",
    (MANIFEST, 2436): "pub(crate) fn validate_v2_session_provenance(",
    (MANIFEST, 2464): "if manifest.transition.fingerprint != content.sha256 {",
    (MANIFEST, 2481): "const MAX_SESSION_SEMANTICS_TRANSITION_PROOF_BYTES: u64 = 4096;",
    (MANIFEST, 2499): "fn bind_v2_provenance_to_durable_proof(",
    (MANIFEST, 2551): "fn binding_io(error: io::Error) -> V2ProvenanceProofBindingError {",
    # Pre-existing production code this seed reasons about.
    (MANIFEST, 256): "session_semantics_version: SessionSemanticsVersion::V1,",
    (MANIFEST, 1365): "self.prepare_compare_and_swap(expected_generation, candidate, true)",
    (MANIFEST, 1375): ".preflight_immutable_exact(provenance_path, &proof_bytes, self.qualification)",
    (MANIFEST, 1749): "return reject(ManifestCasRejection::IdempotencyConflict);",
    (MANIFEST, 1762): "return reject(ManifestCasRejection::TransitionConflict);",
    (MANIFEST, 1768): "return reject(ManifestCasRejection::CompletionRequiresPrepared);",
    (MANIFEST, 1800): "let recovery_key = recovery_key(&candidate.transition.fingerprint);",
    (MANIFEST, 1808): "fn commit_prepared_compare_and_swap(",
    (MANIFEST, 2008): "fn load_manifest_file(path: &Path)",
    (MANIFEST, 2024): "let opened_metadata = file.metadata().map_err(|error| load_io(&error))?;",
    # Tests.
    (MANIFEST, 3893): "fn v2_candidate_requires_exact_bound_session_provenance_proof() {",
    (MANIFEST, 4037): "fn accepted_v2_manifest_cannot_regress_to_v1() {",
    (MANIFEST, 4065): "fn proof_before_manifest_transition_returns_actual_accepted_and_already_completed() {",
    (MANIFEST, 4138): "fn addressed_generic_cas_cannot_install_v2_without_owning_proof_bytes() {",
    (MANIFEST, 4317): "fn committed_v2_head_records_later_generations_only_against_the_durable_proof() {",
    (MANIFEST, 4387): "fn advanced_head_conflicts_on_the_same_idempotency_id_with_changed_artifacts() {",
    (MANIFEST, 4444): "fn quarantine_recovery_remains_closed_on_a_v2_session() {",
    (MANIFEST, 4527): "fn forged_v2_provenance_is_refused_on_an_advanced_head() {",
    (MANIFEST, 4636): "fn advanced_head_refuses_later_generations_when_the_durable_proof_is_not_intact() {",
    (MANIFEST, 4733): "fn proof_conflict_and_indeterminate_prevent_manifest_mutation_then_retry_converges() {",
    (MANIFEST, 4818): "fn stale_existing_head_rejects_different_transition_before_proof_mutation() {",
    # Other modules.
