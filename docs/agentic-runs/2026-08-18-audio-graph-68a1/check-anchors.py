...
EXPECTED: dict[tuple[str, int], str] = {
    ...
    (MANIFEST, 1598): "fn refuse_unproven_v2_candidate(",
    (MANIFEST, 1664): "self.refuse_unproven_v2_candidate(&candidate, proof_owned)",
    (MANIFEST, 2164): "pub(crate) struct V2SessionProvenanceEntry<",
    (MANIFEST, 2169): "pub(crate) fn validate_v2_session_provenance(",
    (MANIFEST, 2197): "if manifest.transition.fingerprint != content.sha256 {",
    (MANIFEST, 2214): "const MAX_SESSION_SEMANTICS_TRANSITION_PROOF_BYTES: u64 = 4096;",
    (MANIFEST, 2232): "fn bind_v2_provenance_to_durable_proof(",
    (MANIFEST, 2284): "fn binding_io(error: io::Error) -> V2ProvenanceProofBindingError {",
    # Pre-existing production code this seed reasons about.
    (MANIFEST, 253): "session_semantics_version: SessionSemanticsVersion::V1,",
    (MANIFEST, 1313): "self.prepare_compare_and_swap(expected_generation, candidate, true)",
    (MANIFEST, 1323): ".preflight_immutable_exact(provenance_path, &proof_bytes, self.qualification)",
    (MANIFEST, 1697): "return reject(ManifestCasRejection::IdempotencyConflict);",
    (MANIFEST, 1710): "return reject(ManifestCasRejection::TransitionConflict);",
    (MANIFEST, 1716): "return reject(ManifestCasRejection::CompletionRequiresPrepared);",
    (MANIFEST, 1748): "let recovery_key = recovery_key(&candidate.transition.fingerprint);",
    (MANIFEST, 1756): "fn commit_prepared_compare_and_swap(",
    (MANIFEST, 1956): "fn load_manifest_file(path: &Path)",
    (MANIFEST, 1972): "let opened_metadata = file.metadata().map_err(|error| load_io(&error))?;",
    # Tests.
    (MANIFEST, 3374): "fn v2_candidate_requires_exact_bound_session_provenance_proof() {",
    (MANIFEST, 3518): "fn accepted_v2_manifest_cannot_regress_to_v1() {",
    (MANIFEST, 3546): "fn proof_before_manifest_transition_returns_actual_accepted_and_already_completed() {",
    (MANIFEST, 3619): "fn addressed_generic_cas_cannot_install_v2_without_owning_proof_bytes() {",
    (MANIFEST, 3798): "fn committed_v2_head_records_later_generations_only_against_the_durable_proof() {",
    (MANIFEST, 3868): "fn advanced_head_conflicts_on_the_same_idempotency_id_with_changed_artifacts() {",
    (MANIFEST, 3916): "fn quarantine_recovery_remains_closed_on_a_v2_session() {",
    (MANIFEST, 3999): "fn forged_v2_provenance_is_refused_on_an_advanced_head() {",
    (MANIFEST, 4108): "fn advanced_head_refuses_later_generations_when_the_durable_proof_is_not_intact() {",
    (MANIFEST, 4205): "fn proof_conflict_and_indeterminate_prevent_manifest_mutation_then_retry_converges() {",
    (MANIFEST, 4290): "fn stale_existing_head_rejects_different_transition_before_proof_mutation() {",
    # Other modules.
