...
- "The transition-proof gate in `prepare_compare_and_swap` is still keyed on the
  ...
  (`src-tauri/src/persistence/session_artifact_manifest.rs:1650`).
- "do not re-attempt the re-key before it lands" and "The wedge is pinned
  ...
  (`src-tauri/src/persistence/session_artifact_manifest.rs:4317`).

What replaced the wedge: `bind_v2_provenance_to_durable_proof`
(`src-tauri/src/persistence/session_artifact_manifest.rs:2499`) reads the durable
...
(`src-tauri/src/persistence/session_artifact_manifest.rs:4444`); the
