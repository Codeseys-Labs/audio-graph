...
| AG-P1-004 | In progress | ASR provider contract cleanup | Keep cloud STT in Rust for `rsac` audio. Deepgram/AssemblyAI already have internal Rust clients; next step is a common transcript/interim event shape. |
...
| AG-P1-006 | Open | Agent/react loop skeleton | Graph-aware action loop that can propose actions, update context, and act with the user. |
