...
/// Event emitted when the tray *Stop capture* menu item is clicked. Re-exported
/// from [`crate::tray`] so the frontend event bridge can reference one name.
/// Routed through the store `stopCapture` path (same as the UI Stop button).
