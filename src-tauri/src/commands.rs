...
/// engine exposes token callbacks; AwsBedrock drives the `ConverseStream` event
/// stream via the on-demand `aws_sdk_bedrockruntime` adapter
/// ([`crate::llm::bedrock`]).
...
/// (Api / OpenRouter / LocalLlama / AwsBedrock), this dispatches to the same
...
/// see no behavior change. For non-streaming providers (MistralRs) this falls
/// through to the legacy blocking executor.
///
...
mod tests {
    ...
    #[test]
    fn streaming_provider_gate_allows_api_openrouter_local_llama_and_bedrock() {
