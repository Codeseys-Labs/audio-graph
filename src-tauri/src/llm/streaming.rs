...
//! `AwsBedrock` is a non-SSE adapter: it builds an `aws_sdk_bedrockruntime`
//! client on demand and drives the `ConverseStream` event stream into
//! `TokenDelta` (see [`crate::llm::bedrock`]). `MistralRs` streaming is still
                    ...
                    .send(TokenDelta::from_terminal_event(StreamTerminalEvent::error(
                        format!(
                            "Streaming chat not yet supported for provider {}; \
                             scoped follow-up issue (LocalLlama/MistralRs).",
