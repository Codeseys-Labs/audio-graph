...
fn default_output_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../src/generated/speechSpanRevision.ts")
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let expected =
        audio_graph_ipc_contract::speech_span_revision::speech_span_revision_typescript_module();
    if check {
        let actual = fs::read_to_string(&output_path)?;
        if actual != expected {
            return Err(format!(
                "speech span revision contract drift detected in {}; run `bun run generate:speech-span-contract`",
                output_path.display()
            )
            .into());
        }
