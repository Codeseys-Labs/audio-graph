...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let expected =
        audio_graph_ipc_contract::endpoint_credential_routing::endpoint_credential_routing_typescript_module();

    if check {
        let actual = fs::read_to_string(&output_path)?;
        if actual != expected {
            return Err(format!(
                "endpoint credential routing contract drift detected in {}; run `bun run generate:endpoint-credential-routing`",
                output_path.display()
            )
            .into());
        }
