...
fn default_output_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../src/generated/sessionDataMovement.ts")
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let expected =
        audio_graph_ipc_contract::session_data_movement::session_data_movement_typescript_module();

    if check {
        let actual = fs::read_to_string(&output_path)?;
        if actual != expected {
            return Err(format!(
                "session data movement contract drift detected in {}; run `bun run generate:session-data-movement`",
                output_path.display()
            )
            .into());
        }
